...
export const generatePDF = async (data: ReportData): Promise<void> => {
  try {
    ...
    try {
      ...
      pdf.setFontSize(12);
      pdf.text("REEWALK CLINIC", pageWidth / 2, currentY + 10, {
        align: "center",
      });
    }
    ...
    pdf.setFontSize(9);
    pdf.text(
      "B4-F4, Vignanpuri Colony, Vidyanagar, Hyderabad - 500044",
      pageWidth / 2,
      currentY,
      { align: "center" }
    );
    ...
    pdf.text(
      "reewalkclinic@gmail.com | +91 9835030442, +91 9337032349",
      pageWidth / 2,
      currentY,
      { align: "center" }
    );
    ...
    pdf.text("Reewalk Foot Care Test Report", pageWidth / 2, currentY, {
      align: "center",
    });
    ...
    const filename = `reewalk_foot_analysis_${data.reportId}.pdf`;
    pdf.save(filename);
